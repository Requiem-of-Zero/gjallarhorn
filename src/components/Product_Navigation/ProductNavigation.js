...
const ProductNavigation = ({ setActive, active, setFiltered, products }) => {
  ...
  return (
    <div className="m0a bg-grey text-grey scrollbar-hide z-10 flex w-full whitespace-nowrap border-b border-t border-[#1F2122] border-solid overflow-x-scroll overflow-y-hidden top-header lg:top-header-desktop sticky top-[38px] max-w-contentContainer sml:justify-center w-full lgl:top-[120px]">
      <ul className="flex gap-[30px] px-[20px] pt-2 tracking-widest text-white font-[100] md:items-center md:justify-center">
        <li
          onClick={() => setActive(0)}
          className={`py-2 border-b-4 cursor-pointer ${active === 0 ? "border-blue" : "border-[transparent]"}`}
        >
          ALL
        </li>
        <li
